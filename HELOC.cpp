...
float helocFinalBalance(const int lineOfCreditMaximum, const float interestRate,
  ...
  if(numberOfYears <= 0 || interestRate < 0 || lineOfCreditMaximum < 0){
    return NAN;
  }
  float monthlyint = (interestRate/12.0)/100.0;
  float oweamount = initialBalance; 
  int numofMonths = numberOfYears*12; 

  for(int x = 0; x < numofMonths; x++){
    if(oweamount < 0){
      oweamount = oweamount - payments[x];
    }
    else{
      oweamount = oweamount + oweamount*monthlyint - payments[x];

      if(oweamount > lineOfCreditMaximum){
        oweamount = oweamount + oweamount*0.01;
      }
    }
...
float totalInterestPaid(const int lineOfCreditMaximum, const float interestRate,
  ...
  for(int x = 0; x < numofMonths; x++){
    if(oweamount < 0){
      ...
      interestpaid += oweamount*monthlyint;
      oweamount = oweamount + oweamount*monthlyint - payments[x];
    

      if(oweamount > lineOfCreditMaximum){
        
        interestpaid += oweamount*0.01; 
        oweamount = oweamount + oweamount*0.01;
      }
